 ...
 * name (carrot.jpg and carrot.svg), the photo is used. NAMES covers names a file name can't spell.
 */
const NAMES: Record<string, string> = { redacted: "[Redacted]" };
...
for (const [path, url] of Object.entries(files)) {
  ...
  byId.set(id, { id, name: NAMES[id] ?? words.charAt(0).toUpperCase() + words.slice(1), url });
}
